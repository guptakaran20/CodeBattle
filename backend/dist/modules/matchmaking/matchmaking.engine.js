...
export class MatchmakingEngine {
    ...
    static async processQueue(difficulty) {
        ...
        for (const userId of members) {
            ...
            const state = await MatchmakingService.getPlayerState(userId);
            if (!state)
                continue; // Player left queue
            const radius = MatchmakingService.getDynamicSearchRadius(state.joinedAt);
            const potentialOpponents = await MatchmakingService.findOpponentsInRange(difficulty, state.elo, radius, userId);
            for (const oppId of potentialOpponents) {
                if (matched.has(oppId))
                    continue;
...
//# sourceMappingURL=matchmaking.engine.js.map