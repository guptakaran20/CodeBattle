...
// Global Feed History
const feedHistory = [];
export const initializeBattleGateway = (io) => {
    io.on('connection', (socket) => {
        ...
        socket.on(SocketEvents.JOIN_ROOM, async (payload) => {
            try {
                ...
                socket.currentBattleCode = battle.battleCode;
                socket.currentBattleId = battle._id.toString();
        ...
        socket.on('disconnect', async () => {
            ...
            const battleCode = socket.currentBattleCode;
            if (!battleCode)
                return;
            ...
            const timeout = setTimeout(async () => {
                ...
                const battleId = socket.currentBattleId;
                if (battleId) {
...
export const BattleGatewayService = {
    ...
    broadcastGlobalFeed: (io, payload) => {
        ...
        if (payload.battleCode) {
            ...
            if (index !== -1)
                feedHistory.splice(index, 1);
        }
        feedHistory.unshift(payload);
        if (feedHistory.length > 5)
            feedHistory.pop();
...
//# sourceMappingURL=battle.gateway.js.map