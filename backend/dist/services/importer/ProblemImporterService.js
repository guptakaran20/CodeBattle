...
export class ProblemImporterService {
    ...
    async runImport(targetEasy = 50, targetMedium = 50, targetHard = 50) {
        ...
        for (const item of list) {
            ...
            if (diff === 'EASY' && easyCount >= targetEasy)
                continue;
            if (diff === 'MEDIUM' && mediumCount >= targetMedium)
                continue;
            if (diff === 'HARD' && hardCount >= targetHard)
                continue;
    ...
    async importSingleProblem(titleSlug) {
    ...
    async logFailure(slug, errors, rawSource, aiLogs, validationTime = 0) {
        await ImportLog.create({
            problemSlug: slug,
...
//# sourceMappingURL=ProblemImporterService.js.map