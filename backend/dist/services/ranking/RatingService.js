...
export class RatingService {
    ...
    static async updateBattleRatings(battleId, winnerId, participantIds, isTimeout = false) {
        ...
        if (users.length === 2) {
            ...
            let s1 = 0.5, s2 = 0.5;
            let out1 = isTimeout ? 'TIMEOUT' : 'DRAW';
            let out2 = isTimeout ? 'TIMEOUT' : 'DRAW';
            if (winnerId) {
    ...
    static async applyRatingChange(user, battleId, delta, outcome) {
        ...
        if (outcome === 'DRAW' || outcome === 'TIMEOUT')
