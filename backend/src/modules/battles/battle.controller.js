...
export const getBattleHistory = async (req, res, next) => {
    try {
...
export const getBattle = async (req, res, next) => {
    try {
        const identifier = req.params.battleCode;
        ...
        if (battle.status === 'IN_PROGRESS' && battle.startTime) {
            ...
            if (Date.now() > endTime) {
                ...
                const participantIds = battle.teams.flatMap((t) => t.members.map((m) => m._id ? m._id.toString() : m.toString()));
                const ratingDeltas = await RatingService.updateBattleRatings(battle._id.toString(), null, participantIds, true);
                await ReplayService.createSummary(battle._id.toString(), null, participantIds, battle.startTime, new Date(), 'COMPLETED', ratingDeltas);
        ...
        if (battleObj.problem && battleObj.problem.testcases) {
            battleObj.problem.testcases = battleObj.problem.testcases.filter((tc) => !tc.isHidden);
        }
...
export const joinBattle = async (req, res, next) => {
    try {
        ...
        if (!team2) {
            battle.teams.push({ teamId: 'team-2', members: [req.user.id] });
        }
        else {
            team2.members.push(req.user.id);
        }
        await battle.save();
...
export const startBattle = async (req, res, next) => {
    try {
        ...
        const allMembers = battle.teams.flatMap((t) => t.members.map((m) => m._id ? m._id.toString() : m.toString()));
        ...
        try {
            ...
            const pNames = battle.teams.flatMap((t) => t.members.map((m) => m.username || 'Player'));
            BattleGatewayService.broadcastGlobalFeed(io, {
                event: 'MATCH_STARTED',
                users: pNames.length > 0 ? pNames : ['Unknown', 'Unknown'],
                difficulty: battle.problem?.difficulty || 'MIXED',
                time: 'Just now'
            });
        }
        catch (e) {
...
export const leaveBattle = async (req, res, next) => {
    try {
        ...
        const allMembers = battle.teams.flatMap((t) => t.members.map((id) => id.toString()));
        if (!allMembers.includes(req.user.id)) {
            return res.status(403).json({ success: false, message: 'You are not a participant' });
        }
        const opponentId = allMembers.find((id) => id !== req.user.id);
        battle.status = 'COMPLETED';
        ...
        await ReplayService.createSummary(battle._id.toString(), opponentId || null, allMembers, battle.startTime || new Date(), new Date(), 'FORFEIT');
        if (opponentId) {
