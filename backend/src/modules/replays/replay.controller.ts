...
export const ReplayController = {
  getSummary: async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const summary = await BattleReplay.findOne({ battleId: battleId as string })
  ...
  getTimeline: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { battleId } = req.params;
      const events = await BattleEvent.find({ battleId: battleId as string }).sort({ sequenceNumber: 1 });
      ...
      events.forEach(e => {
        if (e.payload?.userId) userIdsToFetch.add(e.payload.userId.toString());
        if (e.payload?.creator) userIdsToFetch.add(e.payload.creator.toString());
      });
  ...
  getRawEvents: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { battleId } = req.params;
      const events = await BattleEvent.find({ battleId: battleId as string }).sort({ sequenceNumber: 1 });
