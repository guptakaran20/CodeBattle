...
export class ReplayService {
     ...
     */
    static async logEvent(battleId, eventType, payload = {}) {
        try {
            ...
            try {
                await redis.xadd('battle_events', '*', 'battleId', battleId.toString(), 'sequenceNumber', sequenceNumber.toString(), 'eventType', eventType, 'payload', JSON.stringify(payload), 'timestamp', event.timestamp.toISOString());
            }
            catch (streamErr) {
     ...
     */
    static async createSummary(battleId, winnerId, participantIds, startedAt, endedAt, finalStatus, ratingDeltas) {
        try {
