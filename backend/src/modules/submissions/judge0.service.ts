...
export class Judge0Service {
  static async submitBatch(submissions: any[], webhookUrl?: string): Promise<string[]> {
    const payload = {
    ...
    return response.data.map((item: any) => item.token);
  }

  static async getBatchResults(tokens: string[]): Promise<any[]> {
    const headers: any = {};
    if (JUDGE0_API_KEY) {
      headers['X-RapidAPI-Key'] = JUDGE0_API_KEY;
    }
