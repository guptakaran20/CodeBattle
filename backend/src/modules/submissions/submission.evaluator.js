...
export const evaluateSubmissionResult = async (submissionId, judge0Results) => {
    ...
    if (judge0Results.length === 0) {
        ...
        for (const res of judge0Results) {
            if (res.compile_output)
                compileOutput = Buffer.from(res.compile_output, 'base64').toString('utf-8');
    ...
    if (lastRes) {
        if (lastRes.stdout)
            submission.stdout = Buffer.from(lastRes.stdout, 'base64').toString('utf-8');
        if (lastRes.stderr)
            submission.stderr = Buffer.from(lastRes.stderr, 'base64').toString('utf-8');
    }
    ...
    if (battle.status === 'IN_PROGRESS' && finalStatus === 'ACCEPTED') {
        ...
        if (!battle.winner) {
            ...
            if (allMembers.includes(user._id.toString())) {
                ...
                const updatedBattle = await Battle.findOneAndUpdate({ _id: battle._id, status: 'IN_PROGRESS', winner: { $exists: false } }, {
                    $set: {
                        winner: user._id,
                        status: 'COMPLETED',
                        endTime: new Date(),
                ...
                if (updatedBattle) {
                    ...
                    const participantIds = battle.teams.flatMap((t) => t.members.map((id) => id.toString()));
                    ...
                    await ReplayService.createSummary(battle._id.toString(), user._id.toString(), participantIds, battle.startTime || new Date(), new Date(), 'COMPLETED', ratingDeltas);
...
//# sourceMappingURL=submission.evaluator.js.map