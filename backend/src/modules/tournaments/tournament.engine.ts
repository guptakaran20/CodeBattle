...
export class TournamentEngine {
   ...
   */
  private static async spawnBattle(tournament: any, match: any) {
    ...
    if (!problem) throw new Error("No problems available to spawn tournament battle");
  ...
  static async startTournament(tournamentId: string) {
    ...
    if (tournament.status !== 'CHECK_IN') throw new Error("Tournament must be in CHECK_IN phase to start");
    ...
    await TournamentParticipant.updateMany(
      { tournamentId: tournament._id, status: 'CHECKED_IN' },
      { $set: { status: 'ACTIVE' } }
    );
   ...
   */
  static async advanceWinner(battleId: string, winnerId: string | null) {
    const battle = await Battle.findById(battleId);
    if (!battle || battle.battleType !== 'TOURNAMENT' || !battle.tournamentMatchId) return;
    ...
    if (!advancedUserId) return; // Should not happen

    match.winner = advancedUserId as any;
    await match.save();
    ...
    const io = getIO();
    if (io) io.emit(`tournament_${match.tournamentId}`, { type: 'MATCH_COMPLETED', matchId: match._id });
    ...
    if (match.nextMatchId) {
      ...
      if (nextMatch) {
        ...
        if (!nextMatch.player1) {
          nextMatch.player1 = advancedUserId as any;
        } else if (!nextMatch.player2) {
          nextMatch.player2 = advancedUserId as any;
        }
