...
export class MatchmakingService {
    ...
    static async leaveQueue(userId) {
        ...
        if (!difficultyStr)
            return; // Not in queue
        const queueKey = this.getQueueKey(difficultyStr);
        const pipeline = redis.pipeline();
     ...
     */
    static getDynamicSearchRadius(joinedAt) {
     ...
     */
    static async getQueueMembers(difficulty) {
        const queueKey = this.getQueueKey(difficulty);
     ...
     */
    static async findOpponentsInRange(difficulty, userElo, radius, excludeUserId) {
        const queueKey = this.getQueueKey(difficulty);
        const min = userElo - radius;
        const max = userElo + radius;
        const potentialOpponents = await redis.zrangebyscore(queueKey, min, max);
        return potentialOpponents.filter((id) => id !== excludeUserId);
    }
}
